...
class ProgramGUI:
    ...
    def __init__(self):
        ...
        try:
            with open("data.txt", 'r') as quiz:
                self.data = json.load(quiz)
        except FileNotFoundError:
            # message box window  in case file is unavailable
            tkinter.messagebox.showerror("Missing file", "Missing or invalid file")
            self.window.destroy()
            return  # exit program gracefully

        if len(self.data) < 5:
            # message box insufficient number of questions, less that 5.End program
    ...
    def check_answer(self):
        ...
        if self.current_question > 5:
            tkinter.messagebox.showinfo("Final Score",
        ...
        # clear input field before next query
